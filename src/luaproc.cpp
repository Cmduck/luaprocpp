...
#define luaL_createlib( L, funcs )     { lua_newtable( L ); \
...
struct stchannel {
    ...
    list recv;
    std::mutex mutex;
    std::condition_variable can_be_used;
};
...
/* channel list mutex */
static std::mutex mutex_channel_list;

/* recycle list mutex */
static std::mutex mutex_recycle_list;
...
static std::condition_variable cond_mainls_sendrecv;

/* main state communication mutex */
static std::mutex mutex_mainls;
...
static channel *channel_locked_get( const char *chname ) {
  ...
  /* get exclusive access to channels list */
  std::unique_lock<std::mutex> lock(mutex_channel_list);
  /*
  ...
  if ( dstlp != NULL ) { /* found a receiver? */
    ...
    if ( dstlp->lstate == mainlp.lstate ) {
      /* if sending process is the parent (main) Lua state, unblock it */
        mutex_mainls.lock();
        cond_mainls_sendrecv.notify_one();
        mutex_mainls.unlock();
    } else {
    ...
    if ( L == mainlp.lstate ) {
      ...
      luaproc_unlock_channel( chan );
      std::unique_lock<std::mutex> lock(mutex_mainls);
      cond_mainls_sendrecv.wait(lock);
      lock.unlock();
      return mainlp.args;
    } else {
...
LUALIB_API int luaopen_luaproc( lua_State *L ) {
  ...
  //luaL_createlib( L, luaproc_funcs );
